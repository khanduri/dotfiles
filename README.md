...
Portable public configuration for macOS, managed with GNU Stow. The personal
laptop installs these files directly; another Mac can source the same files from
its own private entry points. No credentials, machine identity, or employer
configuration belongs here.

Tools: **zsh + oh-my-zsh + Powerlevel10k**, **Neovim + lazy.nvim**, **Herdr**, and
...
Install Homebrew first. Clone this repository, then run from the checkout:
...
Install oh-my-zsh without an installer modifying your rc file:

```sh
# Run once, if this directory does not already exist.
git clone https://github.com/ohmyzsh/ohmyzsh.git "$HOME/.oh-my-zsh"
```

Powerlevel10k is supplied by the Brewfile. Open a new zsh, then install Neovim's
plugins once:
...
Open Neovim again after installation. Core editor settings still load if plugins
or language servers are unavailable. Plugin downloads happen only when requested;
plugin state and writable lockfiles live outside the checkout.

Set Git identity locally, never in this repository:
...
The shared Git config includes this file last. Preserve any existing personal Git
overrides there before replacing `~/.gitconfig`.
...
# Include tmux in both Stow commands if you installed that fallback.
nvim --headless '+DotfilesInstall' '+Lazy! sync' +qa
dotfiles-code-extensions
```

Review the pull and dry-run output before applying links. Existing links already
see content changes; restowing handles added and removed package files. Open a
fresh zsh and Neovim. Herdr picks up configuration on restart, or run
`herdr server reload-config` for an existing server. Reload the optional tmux
config with Ctrl-A `r`. Do not stop active sessions just to update configuration.

On a Mac whose **private repo owns the entry points**:
